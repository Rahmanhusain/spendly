...
export async function createAdminSession(
  ...
  const result = await query<AdminSessionRecord>(
    `INSERT INTO admin_sessions
     ...
     RETURNING *`,
    [sessionId, adminId, refreshTokenHash, expiresAt, ipAddress ?? null, userAgent ?? null]
  );
...
export async function createContactInquiry(data: {
  ...
  const result = await query<ContactInquiryRecord>(
    `INSERT INTO contact_inquiries
       (id, sender_name, sender_email, reason, subject, message, status, created_at, updated_at)
     VALUES ($1, $2, $3, $4, $5, $6, 'new', NOW(), NOW())
     RETURNING *`,
    [id, data.sender_name, data.sender_email, data.reason, data.subject, data.message]
  );
  return result.rows[0];
}
...
export interface InquiryListOptions {
  status?: InquiryStatus;
  reason?: InquiryReason;
  search?: string;       // searches id prefix, email, subject
...
export async function listInboundEmails(opts: EmailListOptions = {}): Promise<{
  ...
  if (opts.search) {
    conditions.push(
      `(from_address ILIKE $${idx} OR subject ILIKE $${idx})`
    );
    values.push(`%${opts.search}%`);
    idx++;
  }
...
export async function setTenantStatus(
  ...
  // so currently logged-in users are kicked out immediately on next request
  if (status !== "active") {
    await query(
