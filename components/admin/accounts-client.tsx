...
import { Search, Building2, ChevronLeft, ChevronRight, ArrowRight } from "lucide-react";
...
export function AccountsClient({ rows, total, offset, limit, search, statusFilter }: Props) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-slate-950">Accounts</h1>
                    ...
                    <div className="flex min-w-0 items-center gap-3 sm:gap-4">
                      ...
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-1.5">
                          <p className="text-sm font-semibold text-slate-950">{tenant.name}</p>
                          <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium capitalize ${STATUS_PILL[tenant.status] ?? "bg-slate-100 text-slate-600"}`}>
                            {tenant.status}
                          </span>
                          <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium capitalize ${PLAN_PILL}`}>
                            {tenant.plan}
                          </span>
                        </div>
                        <p className="mt-0.5 truncate text-xs text-slate-500">
                          {tenant.slug} · {tenant.active_user_count}/{tenant.user_count} users · {formatDate(tenant.created_at)}
                        </p>
                      </div>
                    </div>
