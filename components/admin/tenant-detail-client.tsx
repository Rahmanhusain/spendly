...
import { Search, ChevronLeft, ChevronRight, ArrowLeft, User, AlertTriangle } from "lucide-react";
      ...
      <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 shadow-xl">
        ...
        <div className="mt-5 flex justify-end gap-3">
          ...
          <Button size="sm" onClick={onConfirm} disabled={loading}
            ...
            {loading
              ? <span className="flex items-center gap-2"><span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-white/30 border-t-white" />Confirming…</span>
              : "Confirm"}
          </Button>
        </div>
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 shadow-xl">
        ...
        <p className="mt-1 text-sm text-slate-600">
          New password for <strong>{userName}</strong>. All their sessions will be revoked.
        </p>
        <Input
          type="password"
        ...
        <div className="mt-4 flex justify-end gap-3">
          ...
          <Button size="sm" onClick={() => onConfirm(pw)} disabled={loading || pw.length < 8}
            className="rounded-full bg-slate-950 text-white hover:bg-slate-800 disabled:opacity-60">
...
export function TenantDetailClient({ tenant, users, total, offset, limit, search }: Props) {
  ...
  async function patchTenant(status: "active" | "inactive") {
    ...
    setActionError(null);
    try {
      const res = await fetch(`/api/admin/accounts/tenants/${tenant.id}`, {
        method: "PATCH",
  ...
  async function patchUserPassword(userId: string, newPassword: string) {
    ...
    setActionError(null);
    try {
      const res = await fetch(`/api/admin/accounts/users/${userId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-1.5">
                <div className="flex flex-wrap items-center gap-2">
                  <CardTitle className="text-xl text-slate-950">{tenant.name}</CardTitle>
                  <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${TENANT_STATUS_PILL[tenant.status] ?? "bg-slate-100 text-slate-600"}`}>
                    {tenant.status}
                  </span>
          ...
          <CardContent className="p-0">
            <div className="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
              <p className="text-sm font-semibold text-slate-950">
                Users <span className="font-normal text-slate-500">({total})</span>
              </p>
                ...
                {users.map((user) => {
                  ...
                  return (
                    <li key={user.id}
                      ...
                      <div className="flex min-w-0 items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-1.5">
                            ...
                            <Badge className={`border-0 text-xs ${USER_STATUS_PILL[user.status] ?? "bg-slate-100 text-slate-600"}`}>
                              {user.status}
                            </Badge>
                      ...
                      <Button size="sm" variant="outline"
                        onClick={() => setDialog({ type: "user_password", userId: user.id, userName: displayName })}
