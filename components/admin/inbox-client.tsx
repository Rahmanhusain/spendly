...
import { Search, Mail, MailOpen, ChevronLeft, ChevronRight } from "lucide-react";
...
export function InboxClient({
  ...
  async function handleMarkRead(email: InboundEmailRecord) {
    ...
    await fetch(`/api/admin/inbox/${email.id}/read`, { method: "POST" });
    setMarkingRead(null);
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-48">
          ...
          <Input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                navigate({ search: (e.target as HTMLInputElement).value, offset: "0" });
              }
            }}
          />
        </div>
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-start gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p className={`truncate text-sm ${email.is_read ? "text-slate-600" : "font-semibold text-slate-950"}`}>
                              {email.from_address}
                            </p>
                            <p className={`mt-0.5 truncate text-sm ${email.is_read ? "text-slate-500" : "text-slate-700"}`}>
                              {email.subject}
                            </p>
                            {email.text_body && (
          ...
          <CardContent className="p-0">
            {!selected ? (
              <div className="flex flex-col items-center gap-2 py-16 text-center">
                ...
                <p className="text-sm text-slate-500">Select an email to read it.</p>
              </div>
            ) : (
              <div className="p-5 space-y-4">
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-slate-600">
                    <span className="font-medium">From:</span> {selected.from_address}
                  </p>
                  <p className="text-sm text-slate-600">
                    <span className="font-medium">To:</span> {selected.to_address}
                  </p>
