...
import { Inbox, MessageSquare, Users, Building2, ArrowRight, Circle } from "lucide-react";
import { RefreshButton } from "@/components/admin/refresh-button";

async function getStats() {
  const [inquiries, emails, tenants, users, recentInquiries] = await Promise.all([
    ...
    query<{ id: string; sender_name: string; subject: string; reason: string; status: string; created_at: string }>(
          ...
          <Link key={title} href={href} className="group block">
            <Card className="border-slate-200 shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 pt-5 px-5">
                ...
                <div className={`flex h-8 w-8 items-center justify-center rounded-full ${alert ? "bg-slate-950" : "bg-slate-100"}`}>
                  <Icon className={`h-4 w-4 ${alert ? "text-white" : "text-slate-500"}`} />
                </div>
              </CardHeader>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
            <div>
              <CardTitle className="text-base text-slate-950">Recent inquiries</CardTitle>
              <CardDescription className="mt-0.5">Latest contact form submissions.</CardDescription>
            </div>
                  ...
                  <li key={inq.id}>
                    <Link href={`/admin/inquiries`}
                      ...
                      <div className="flex items-start gap-3 min-w-0">
                        <Circle className={`mt-1.5 h-2 w-2 shrink-0 ${inq.status === "new" ? "fill-slate-950 text-slate-950" : "fill-slate-300 text-slate-300"}`} />
                        <div className="min-w-0">
                          <p className="truncate text-sm font-medium text-slate-950">{inq.subject}</p>
                          <p className="mt-0.5 truncate text-xs text-slate-500">
                            {inq.sender_name} · {REASON_LABELS[inq.reason] ?? inq.reason}
                          </p>
                        </div>
                      </div>
                      <div className="flex shrink-0 flex-col items-end gap-1">
                        <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_PILL[inq.status] ?? "bg-slate-100 text-slate-600"}`}>
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardHeader className="border-b border-slate-100 pb-4">
            <CardTitle className="text-base text-slate-950">Quick links</CardTitle>
            <CardDescription className="mt-0.5">Jump to common tasks.</CardDescription>
          </CardHeader>
          <CardContent className="p-0">
            {[
              { href: "/admin/inbox", label: "View inbox", sub: `${stats.emails.unread} unread`, icon: Inbox },
              { href: "/admin/inquiries?status=new", label: "New inquiries", sub: `${stats.inquiries.new} pending`, icon: MessageSquare },
              { href: "/admin/accounts", label: "All workspaces", sub: `${stats.tenants.total} tenants`, icon: Building2 },
              { href: "/admin/accounts", label: "User accounts", sub: `${stats.users.active} active`, icon: Users },
